...
def ban_user(user_id: int, reason: str, days: int = None):
    conn = sqlite3.connect("/opt/render/project/src/bot.db")  # путь к базе основного бота
    c = conn.cursor()
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ban_"):
        ...
        await query.edit_message_text(
            f"⛔ Пользователь <a href='tg://user?id={target_id}'>{target_id}</a> забанен ({reason_text}).",
            parse_mode="HTML"
        )
...
async def ban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"⛔ Пользователь <a href='tg://user?id={target}'>{target}</a> забанен ({reason_text}).",
        parse_mode="HTML"
    )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(callback_handler, pattern="^ban_"))
